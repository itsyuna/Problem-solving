function solution(n, lost, reserve) {
  ...
  const losted = [...lost].filter(elem => !reserve.includes(elem));
  const reserved = [...reserve].filter(elem => !lost.includes(elem));

  // 현재 시점 체육 수업을 들을 수 있는 학생
  // -> 전체 학생 수 - 체육복을 도난당한 학생 수
  const fixedStudents = n - losted.length;

  let count = 0;

  // losted 배열은 작은 수부터 순회할 수 있게 오름차순 정렬
  // Ex) solution(5, [4, 2], [3, 5])); // return -> 5
  const newLost = losted.sort((a, b) => a - b);

  // 빌려줄 수 있는 학생 수 체크
  for (const eachLost of newLost) {
    if (reserved.includes(eachLost - 1)) {
      const idx = reserved.indexOf(eachLost - 1);
